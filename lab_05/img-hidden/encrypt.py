...
def encode_image(image_path, message):
    ...
    for row in range(height):
        for col in range(width):
            ...
            for color_channel in range(3):  # Xử lý từng kênh màu (RGB)
            ...
            if data_index >= len(binary_message):  # Thoát khi mã hóa xong
                break
        if data_index >= len(binary_message):
            break
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python encode.py <image_path> <message>")
        return
